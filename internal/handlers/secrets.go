...
func (h *SecretHandler) CreateSecret(ctx context.Context, request api.CreateSecretRequestObject) (api.CreateSecretResponseObject, error) {
	...
	if (isAutoGenerated && (!configProvided || dataProvided)) || (!isAutoGenerated && (configProvided || !dataProvided)) || !isValidType {
		span.SetStatus(codes.Error, "Wrong request format")
		logger.Warn("Wrong request format", slog.Any("request_type", string(request.Body.Type)), slog.Any("data_provided", dataProvided), slog.Any("config_provided", configProvided))
		return BuildCreateSecretErrorResponse(ErrorResult{
...
func (h *SecretHandler) UpdateSecret(ctx context.Context, request api.UpdateSecretRequestObject) (api.UpdateSecretResponseObject, error) {
	...
	isAutoGenerated := *request.Body.Type == api.UpdateSecretRequestTypeAutoGenerated
	dataProvided := request.Body.Data != nil && len(*request.Body.Data) > 0
	configProvided := request.Body.GenerationConfig != nil

	if (isAutoGenerated && (!configProvided || dataProvided)) || (!isAutoGenerated && (configProvided || !dataProvided)) {
		return BuildUpdateSecretErrorResponse(ErrorResult{
			ErrorMessage: "Wrong request format: Data and GenerationConfig are mutually exclusive based on Secret Type",
			ErrorCode:    "BadRequest",
			StatusCode:   400,
		}), nil
	}
